...
export function ensureCameraShotCard(beat, { index = 0, prev = null } = {}) {
  ...
  // Variety vs previous same-room beat
  if (
    prev &&
    ...
    const prevSize =
      ...
      "medium_full";
    if (prevSize === shotSize) {
      const cycle = ["wide", "medium_full", "medium", "close"];
      const idx = Math.max(0, cycle.indexOf(shotSize));
      shotSize = cycle[(idx + 1 + (index % 2)) % cycle.length];
    }
  }
  ...
  if (!anchors[anchorName]) {
    // Pick a non-center anchor periodically for variety
...
function endCameraFromMotion(motion, { shotSize, offset, zoom }) {
  ...
  if (m === "push_in") {
    endZoom = clamp(zoom * 1.14, 1, 1.7);
    endShot = shotSize === "wide" ? "medium" : shotSize === "medium_full" ? "close" : "close";
    endOffset = { x: offset.x * 0.85, y: offset.y - 0.04 };
  } else if (m === "pull_back") {
    endZoom = clamp(zoom * 0.9, 1, 1.5);
    endShot = "wide";
    endOffset = { x: offset.x * 0.5, y: offset.y + 0.03 };
  } else if (m === "track") {
    endOffset = {
      x: clamp(offset.x + 0.12, -0.45, 0.45),
      y: offset.y,
    };
  } else if (m === "lower") {
    endOffset = { x: offset.x, y: clamp(offset.y + 0.08, -0.35, 0.35) };
    endZoom = clamp(zoom * 1.06, 1, 1.6);
  } else if (m === "hold_wide") {
    endZoom = Math.min(zoom, 1.08);
    endShot = "wide";
  } else {
    // Gentle living drift
    endZoom = clamp(zoom * 1.03, 1, 1.55);
    endOffset = { x: offset.x, y: clamp(offset.y - 0.02, -0.35, 0.35) };
  }
...
export function applyCameraShotCards(beats) {
  ...
  // Second pass: ban 3 identical consecutive shot sizes in same room
  for (let i = 2; i < list.length; i++) {
    const a = list[i - 2];
    const b = list[i - 1];
    const c = list[i];
    if (c.bridge || b.bridge || a.bridge) continue;
    const loc = String(c.location || "").toLowerCase();
    if (
      loc &&
      loc === String(b.location || "").toLowerCase() &&
      loc === String(a.location || "").toLowerCase()
    ) {
      const sa = a.camera?.shotSize;
      const sb = b.camera?.shotSize;
      const sc = c.camera?.shotSize;
      if (sa && sa === sb && sb === sc) {
        c.camera = {
          ...c.camera,
          shotSize: sa === "wide" ? "medium" : "wide",
          zoom: sa === "wide" ? 1.22 : 1.0,
        };
        c.cameraFraming = c.camera.shotSize === "wide" ? "full_body" : "medium";
      }
    }
  }
  return list;
