...
export class MvidOrchestrator extends EventEmitter {
  ...
  setupFlags() {
    ...
    // Only pass --locations when the user explicitly chose a subset
  ...
  getState() {
    return {
      ...
      stages: STAGES,
      setup: this.setup,
      progress: this.progress || this.computeSongProgress(this.tabs),
      gpu: gpuStatus(),
      comfyUrl: this.comfyUrl,
  ...
  computeSongProgress(tabs = this.tabs) {
    ...
    const steps = [
      ...
      },
      {
        tab: "lyrics",
        label: "Lyrics",
        done: !!(t.lyrics?.text && String(t.lyrics.text).trim()),
      },
      {
        tab: "song",
        label: "Song",
      ...
      },
      {
        tab: "storyline",
        label: "Storyline",
        done: !!(t.storyline?.beats?.length || t.scripts?.beats?.length),
      },
      {
        tab: "scenes",
        label: "Room plates",
        done: !!(t.scenes?.locations?.length),
      },
      {
        tab: "scripts",
        label: "Scripts",
        done: !!(t.scripts?.beats?.length),
      },
      {
        tab: "keyframes",
        label: "Keyframes",
        done: !!(t.keyframes?.images?.length),
      },
      {
        tab: "clips",
        label: "Clips",
        done: !!(t.clips?.videos?.length),
      },
      {
        tab: "final",
        label: "Final",
  ...
  _autoApprovePayload(stage) {
    if (stage === "setup") {
      return {
        ...
        locationIds: [...(this.setup?.locationIds || [])],
        locationsExplicit: this.setup?.locationsExplicit === true,
  ...
  async waitForApproval(stage) {
    ...
    if (this.autoApprove && !this._paused && !this._stopped) {
      ...
      return {
        action: "approve",
        payload: this._autoApprovePayload(stage),
        stage,
      };
    }
