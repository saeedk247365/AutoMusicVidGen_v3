...
import { ensureOllamaRunning, DEFAULT_OLLAMA_URL } from "./ensure-ollama.js";
...
import { suggestBrief } from "./suggest-brief.js";
import { packageYouTube } from "./youtube-package.js";
...
export class MvidOrchestrator extends EventEmitter {
  ...
  /** Apply cast/rooms/brief. Does not approve the setup gate. */
  applySetup(body = {}) {
    ...
    this.setup = {
      ...
      style: String(body.style || this.setup?.style || "").trim(),
      roomsLockedByUser:
        body.roomsLockedByUser === true ||
        (hasLocField &&
          body.roomsLockedByUser !== false &&
          this.setup?.roomsLockedByUser === true),
      noFrameChain:
        body.noFrameChain === true ||
        body.noFrameChain === "true" ||
        body.noFrameChain === 1,
      // Default ON. Legacy drafts may have frameChain:false from the old opt-in
      // default — only honor false when noFrameChain is set (explicit opt-out).
      frameChain: !(
        body.noFrameChain === true ||
        body.noFrameChain === "true" ||
        body.noFrameChain === 1
    ...
    saveSetupDraft(this.setup).catch(() => {});
    if (this.songDir) this.persistSession().catch(() => {});
    this.rememberActiveProject().catch(() => {});
    this.emit("state", this.getState());
    return { ok: true, setup: this.setup };
  }

  async hydrateSetupFromDisk() {
    // Prefer song session, then active-project setup, then global draft
    if (this.songDir) {
      const sessionPath = join(this.songDir, "mvid-session.json");
      if (existsSync(sessionPath)) {
        try {
          const data = JSON.parse(stripBom(await readFile(sessionPath, "utf8")));
          if (data?.castIds?.length) {
            this.applySetup({
              ...data,
              locationsExplicit:
                data.locationsExplicit === true ||
                (Array.isArray(data.locationIds) && data.locationIds.length > 0),
            });
            return this.setup;
          }
        } catch {
    ...
    if (active?.setup?.castIds?.length) {
      this.applySetup(active.setup);
      return this.setup;
    }
    const draft = await loadSetupDraft();
    if (draft?.castIds?.length) {
  ...
  setupFlags() {
    ...
    // Always pass explicit room picks; never silently expand to all scenes
    if (this.setup.locationsExplicit && this.setup.locationIds?.length) {
  ...
  /** Flags shared by 02_0 / 02_1 / 02_2 kids-hit stages. */
  kidsHitFlags() {
    const args = [
      ...(this.kidsHit ? ["--kids-hit"] : []),
      ...this.resolutionFlags(),
    ];
    const extras = Array.isArray(this.extraArgs) ? this.extraArgs : [];
    const forceOff =
      this.setup?.noFrameChain === true ||
      extras.includes("--no-frame-chain");
    // Kids-hit defaults to last-frame chain; opt out with --no-frame-chain
    if (this.kidsHit && !forceOff) {
      args.push("--frame-chain");
    }
    return args;
  }
  ...
  getState() {
    return {
      ...
      stages: STAGES,
      setup: this.setup,
      progress: this.computeSongProgress(this.tabs),
      gpu: gpuStatus(),
      comfyUrl: this.comfyUrl,
  ...
  /**
   * Infer how far a song folder has progressed from tab payloads on disk.
   */
  computeSongProgress(tabs = this.tabs) {
    const t = tabs || {};
    const hasSongFolder = !!(this.songDir || this.songRel);
    // Setup counts as done once cast is chosen; later stages only when
    // this project actually has that artifact (not leftover UI state).
    const steps = [
      {
        tab: "setup",
        label: "Cast & Rooms",
      ...
      },
      {
        ...
        label: "Lyrics",
          ...
          hasSongFolder &&
          !!(t.lyrics?.text && String(t.lyrics.text).trim()),
      },
      {
        tab: "song",
        label: "Song",
        done: hasSongFolder && !!t.song?.url,
      },
      {
        tab: "storyline",
        label: "Storyline",
          ...
          hasSongFolder &&
          !!(t.storyline?.beats?.length || t.scripts?.beats?.length),
      },
      {
        tab: "scenes",
        label: "Room plates",
        done: hasSongFolder && !!(t.scenes?.locations?.length),
      },
      {
        tab: "scripts",
        label: "Scripts",
        done: hasSongFolder && !!(t.scripts?.beats?.length),
      },
      {
        tab: "keyframes",
        label: "Keyframes",
        done: hasSongFolder && !!(t.keyframes?.images?.length),
      },
      {
        tab: "clips",
        label: "Clips",
        done: hasSongFolder && !!(t.clips?.videos?.length),
      },
      {
        tab: "final",
        label: "Final",
        done: hasSongFolder && !!t.final?.url,
      },
    ];
    const completedTabs = steps.filter((s) => s.done).map((s) => s.tab);
    let furthest = steps[0];
    for (const s of steps) {
      if (s.done) furthest = s;
    }
    const complete = !!t.final?.url;
    const next = steps.find((s) => !s.done) || null;
    return {
      complete,
      furthestTab: furthest.tab,
      furthestLabel: furthest.label,
      nextTab: next?.tab || null,
      ...
      steps: steps.map((s) => ({
        ...
        current: s.tab === furthest.tab,
      })),
      summary: complete
        ? "Complete"
        : `Reached ${furthest.label}` + (next ? ` · next: ${next.label}` : ""),
    };
  }
   ...
   */
  async openSong(relOrPath, { viewOnly = true } = {}) {
    ...
    this._paused = false;
    this._stopped = false;
    await this.hydrateSetupFromDisk();
    await this.refreshTabsFromDisk();
    const progress = this.computeSongProgress(this.tabs);
    ...
    if (progress.complete) {
      ...
      this.setStage(
        "idle",
        `Viewing ${this.songRel} · ${progress.summary}`,
      );
    }
    this.emit("state", this.getState());
    this.emit("sse", {
      type: "log",
      message: `Opened ${this.songRel} · ${progress.summary}`,
    });
    return {
      ok: true,
      songDir: this.songRel,
   ...
   * Optional fromStage: "song"|"plan"|"keyframes"|"clips"|"final" forces resume
   * at that stage (skips regenerating earlier stages; may remake the named one).
   */
  async continuePipeline({ fromStage = null } = {}) {
    ...
    this._stopped = false;
    this.error = null;
    this._forceFromStage = fromStage
      ? String(fromStage).replace(/^await_/, "").toLowerCase()
      : null;
    this.syncGpuRoute();
    this.emit("sse", {
      type: "log",
      message:
        `Continuing pipeline for ${this.songRel}` +
        (this._forceFromStage ? ` · from ${this._forceFromStage}` : "") +
        ` · GPU ${getGpuBackend()}`,
    });
    this.start().catch((err) => {
      console.error("Continue pipeline failed:", err.message || err);
    });
    return { ok: true, songDir: this.songRel, fromStage: this._forceFromStage };
  }

  /**
   * Fill title / theme / objective (+ default rooms unless user locked rooms).
   * Safe while waiting at await_setup — does not restart the pipeline.
   */
  applySuggestedBrief(opts = {}) {
    const brief = suggestBrief({
      avoidTheme: opts.avoidTheme ?? this.setup?.theme,
      avoidTitle: opts.avoidTitle ?? this.setup?.title,
    });
    const roomsLocked = this.setup?.roomsLockedByUser === true;
    const next = {
      ...(this.setup || {}),
      title: brief.title,
      theme: brief.theme,
    ...
    if (!roomsLocked && Array.isArray(brief.locationIds) && brief.locationIds.length) {
      next.locationIds = brief.locationIds;
      next.locationsExplicit = true;
      next.roomsLockedByUser = false;
    }
    this.setup = next;
    this.tabs.setup = { ...this.setup };
    saveSetupDraft(this.setup).catch(() => {});
    ...
    this.emit("sse", {
      type: "log",
      message:
        `Brief: “${brief.title}” · ${brief.theme}` +
        (!roomsLocked && brief.locationIds?.length
          ? ` · rooms: ${brief.locationIds.join(", ")}`
          : ""),
    });
    this.emit("sse", { type: "brief", ...brief, setup: this.setup });
    return { ok: true, brief, setup: this.setup };
  }

  /**
   * Clear active project and start a fresh setup → lyrics pipeline.
   * If already waiting on setup, just shuffle a new brief instead.
   * Optional body: castIds, locationIds, title/theme/objective to preserve UI picks.
   */
  async startNewProject(body = {}) {
    // Already on setup gate → reshuffle title/theme without killing the wait
    if (this.stage === "await_setup" && this._running) {
      // Keep cast/rooms from the UI when provided
      if (body.castIds || body.locationIds) {
        this.applySetup({
          ...this.setup,
          ...body,
          locationsExplicit:
            body.locationsExplicit === true ||
            this.setup?.locationsExplicit === true ||
            (Array.isArray(body.locationIds) && body.locationIds.length > 0),
        });
      }
      const result = this.applySuggestedBrief();
      return { ok: true, reshuffled: true, ...result };
    }
    if (this._running) {
    ...
    this.songRel = null;
    this.tabs = blankTabs();
    this.progress = this.computeSongProgress(this.tabs);
    const brief = suggestBrief();
    const castIds = Array.isArray(body.castIds) && body.castIds.length
      ? body.castIds.map(String).filter(Boolean)
      : ["adam", "sasha"];
    const roomsLocked = body.roomsLockedByUser === true;
    let locationIds = Array.isArray(body.locationIds)
      ? body.locationIds.map(String).filter(Boolean)
      : [];
    // Auto-pick rooms from theme unless user already chose
    if (!locationIds.length || !roomsLocked) {
      if (!roomsLocked) {
        locationIds = brief.locationIds?.length
          ? brief.locationIds
          : locationIds;
      }
    }
    const locationsExplicit = locationIds.length > 0;
    this.setup = {
      castIds,
      locationIds,
      locationsExplicit,
      roomsLockedByUser: roomsLocked,
      title: String(body.title || "").trim() || brief.title,
      objective: String(body.objective || "").trim() || brief.objective,
      theme: String(body.theme || "").trim() || brief.theme,
      style: String(body.style || "").trim() || brief.style || "",
      frameChain: true,
      noFrameChain: false,
    };
    // If UI didn't send a brief, use the generated one fully
    if (!body.title && !body.theme) {
      this.setup.title = brief.title;
      this.setup.theme = brief.theme;
      this.setup.objective = brief.objective;
      this.setup.style = brief.style || "";
      if (!roomsLocked && brief.locationIds?.length) {
        this.setup.locationIds = brief.locationIds;
        this.setup.locationsExplicit = true;
      }
    }
    this._started = false;
    ...
    this.emit("sse", {
      type: "log",
      message:
        `Starting new project — “${this.setup.title}” (${this.setup.theme})` +
        (locationIds.length
          ? ` · rooms: ${locationIds.join(", ")}`
          : " · rooms: (pick on Setup)"),
    });
    this.emit("sse", {
      type: "brief",
      ...
      objective: this.setup.objective,
      style: this.setup.style,
      setup: this.setup,
    });
    this.start().catch((err) => {
  ...
  setAutoApprove(enabled) {
    ...
    this.emit("sse", {
      ...
      stage: this.stage?.startsWith("await_")
        ? this.stage.replace(/^await_/, "")
        : null,
    });
    if (this.autoApprove && this._approval && !this._paused && !this._stopped) {
      const stage = String(this.stage || "").replace(/^await_/, "") || "unknown";
      const payload = this._autoApprovePayload(stage);
      this._approval.resolve({ action: "approve", payload, stage });
      this._approval = null;
    }
  }

  /** Payload used when auto-approving a gate (setup needs cast/rooms). */
  _autoApprovePayload(stage) {
    if (stage === "setup") {
      const locationIds = [...(this.setup?.locationIds || [])];
      // Never auto-approve setup with zero rooms — wait for the UI payload
      if (this.setup?.locationsExplicit && !locationIds.length) {
        return null;
      }
      return {
        castIds: [...(this.setup?.castIds || [])],
        locationIds,
        locationsExplicit:
          this.setup?.locationsExplicit === true || locationIds.length > 0,
        title: this.setup?.title || "",
        objective: this.setup?.objective || "",
        theme: this.setup?.theme || "",
        frameChain: this.setup?.frameChain !== false,
        noFrameChain: this.setup?.noFrameChain === true,
      };
    }
    return {};
  }
  ...
  async waitForApproval(stage) {
    ...
    if (this._stopped) return { action: "reject", stage: "stop" };

    // Register the waiter BEFORE emitting await_* so a concurrent
    // Auto-approve checkbox click cannot miss the resolve hook.
    let resolveFn;
    const waitPromise = new Promise((resolve) => {
      resolveFn = resolve;
    });
    this._approval = { resolve: resolveFn, stage };

    this.setStage(`await_${stage}`, `Waiting for approval: ${stage}`);

    if (this.autoApprove && !this._paused && !this._stopped) {
      const payload = this._autoApprovePayload(stage);
      // Setup with no rooms yet → wait for UI approve (don't empty-approve)
      if (stage === "setup" && payload == null) {
        return waitPromise;
      }
      this._approval = null;
      this.emit("sse", { type: "auto_approve", stage, enabled: true });
      return {
        action: "approve",
        payload: payload || {},
        stage,
  ...
  async _ensureOllama() {
    ...
    this.emit("sse", {
      type: "log",
      message: `Checking Ollama at ${url}…`,
    });
    await ensureOllamaRunning(url, {
      onLog: (message) => this.emit("sse", { type: "log", message }),
    });
  }
  ...
  async refreshTabsFromDisk() {
    ...
    if (existsSync(clipsDir)) {
      const files = (await readdir(clipsDir))
        ...
        .sort();
      const beatByStem = new Map();
      for (const b of tabs.scripts?.beats || tabs.storyline?.beats || []) {
        const stem = b.keyframeStem || `${String(b.index || "").padStart(2, "0")}_${b.id}`;
        const dur =
          b.startSec != null && b.endSec != null
            ? Math.max(0.1, Number(b.endSec) - Number(b.startSec))
            : 0;
        if (stem) beatByStem.set(stem, dur);
        if (b.id) beatByStem.set(String(b.id), dur);
      }
      tabs.clips = {
    ...
    if (existsSync(finalPath)) {
      ...
      const ytMetaPath = join(dir, "youtube", "metadata.json");
      if (existsSync(ytMetaPath)) {
        try {
          const yt = JSON.parse(stripBom(await readFile(ytMetaPath, "utf8")));
          tabs.final.youtube = {
            title: yt.title || "",
            description: yt.description || "",
            thumbnailUrl: `/media/youtube/thumbnail.jpg`,
            durationSec: yt.durationSec,
            introSec: yt.introSec,
          };
        } catch {
          /* ignore */
        }
    ...
    this.tabs = tabs;
    // Always refresh progress so lyrics/song greens update mid-pipeline
    this.progress = this.computeSongProgress(tabs);
    this.emit("sse", {
      type: "tabs",
      tabs: this.tabs,
  ...
  async remakeKeyframe({
    ...
    const kidsFlags = this.kidsHitFlags();
  ...
  async _run() {
    ...
    if (this.songArg) {
      ...
      await this.rememberActiveProject();
      await this.refreshTabsFromDisk();
      const cont = await this._continueFromExisting(kidsFlags);
      if (!cont?.fresh) return;
      // Empty husk — fall through into setup → lyrics like a new project
    }

    // ── Setup (cast + rooms + optional title/objective) ──
    {
      await this.hydrateSetupFromDisk();
      if (!this.setup?.title || !this.setup?.theme) {
        this.applySuggestedBrief();
      }
      this.tabs.setup = { ...this.setup };
      this.emit("state", this.getState());
      ...
      if (!this.setup.locationIds?.length) {
        throw new Error(
          "Setup requires at least one room — select rooms on the Setup tab",
        );
      }
      // Mark explicit so --locations is always forwarded
      this.setup.locationsExplicit = true;
      this.setStage(
        "setup",
        `Cast: ${this.setup.castIds.join(", ")} · Rooms: ${this.setup.locationIds.join(", ")}`,
      );
    }
  ...
  async _continueFromExisting(kidsFlags) {
    // fromStage is advisory (logging / future UI); never remake completed stages here.
    this._forceFromStage = null;
    ...
    // Empty husk (e.g. leftover temp dirs only) — fall through to new-song flow
    if (!hasLyrics && !hasMp3 && !hasPlan && !hasKf && !hasClips && !hasFinal) {
      this.emit("sse", {
        type: "log",
        message: `${this.songRel} has no lyrics/song/plan — starting a new song instead`,
      });
      this.songArg = null;
      this.songDir = null;
      this.songRel = null;
    ...
    if (!hasLyrics) {
      throw new Error(
        `Existing song folder has no lyrics.txt (${this.songRel}). Pick another batch or click New.`,
      );
    }
  ...
  async _animateAndStitch(kidsFlags) {
    ...
    try {
      this.setStage("final", "Packaging YouTube title / thumbnail / description…");
      await packageYouTube(this.songDir, {
        log: (m) => this.emit("log", { line: String(m) }),
      });
      await this.refreshTabsFromDisk();
    } catch (err) {
      this.emit("log", {
        line: `  YouTube package warning: ${err?.message || err}`,
      });
    }
