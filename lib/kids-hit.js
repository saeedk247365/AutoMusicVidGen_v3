...
- GEOGRAPHY LOCK: lyric places MUST match the objective/theme rooms. If the objective is wash hands / kitchen / eat, NEVER sing bed, blanket, sheets, pillow, lawn, yard, or outdoor play. Problem = dirty hands / can't eat yet / need soap — sung at the sink/kitchen.
...
- Cast: prefer "I"/"my"; Mom only as a short sung cue ("Mom says go") — never stage directions
 ...
 */
export function stabilizeCastBlocks(beats, { minOn = 3, maxOn = 4, minOff = 2 } = {}) {
  const list = Array.isArray(beats) ? beats.map((b) => ({ ...b })) : [];
  if (list.length < 2) return list;

  const hasHelper = (b) =>
    !!(
      b?.characters?.some((c) => /^sasha$/i.test(c?.name)) ||
      b?.placement?.Sasha ||
      b?.placement?.sasha
    );
  ...
  const cloneHelper = (from, into) => {
    const src =
      from.characters?.find((c) => /^sasha$/i.test(c?.name)) || {
        name: "Sasha",
        pose: "stand",
    ...
    const adam =
      into.characters?.find((c) => /^adam$/i.test(c?.name)) ||
      into.characters?.[0] || {
        name: "Adam",
        pose: "stand",
        expression: "curious",
    ...
    into.characters = [adam, { ...src, name: "Sasha" }];
    into.placement = {
      ...(into.placement || {}),
      Adam: into.placement?.Adam || "mid_left",
      Sasha: into.placement?.Sasha || "mid_right",
    };
  ...
  const dropHelper = (into) => {
    ...
    into.characters = [{ ...adam, name: "Adam" }];
    if (into.placement) {
      const { Sasha, sasha, ...rest } = into.placement;
      into.placement = rest;
    }
...
export function repairKidsHitBeats(
  ...
  // Hold Mom on for multi-beat blocks (same room) instead of flicker in/out
  list = stabilizeCastBlocks(list, { minOn: 3, maxOn: 4, minOff: 2 });
...
function scrubStaleGeographyText(beats, { indoor = false, objective = "" } = {}) {
