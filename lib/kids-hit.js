...
import {
  applyCameraShotCards,
  snapBeatTimingsToMusic,
  buildMusicMap,
  writeMusicMap,
  loadMusicMap,
  musicalCrossfadeSec,
...
export const KIDS_HIT_BEAT_MIN = 12;
/** Prefer fewer, longer same-room clips for smoother Wan chains. */
export const KIDS_HIT_BEAT_MAX = 16;
...
Write a SHORT home SINGALONG (about 75 seconds when sung). It must SOUND LIKE A SONG kids chant — not a story narrating a music video.
...
The whole song has ONE goal a toddler can say out loud.
Write it on the first line after TITLE as:
OBJECTIVE: ...

SONGCRAFT (required):
- Write LYRICS a child would SING — short chantable lines, rhyme + rhythm, repeated hooks.
- Chorus must REPEAT the same 2–4 lines (or a tiny variation) so it is catchy.
- Prefer "I clap / I hop / shoes on / wash wash" over "Mom kneels by the doorway then I notice…"
- Do NOT narrate camera, poses, or scene blocking (no "Mom kneels nearby", "I look up at Mom", "we stand side by side" as stage directions).
- Tiny story is fine ONLY as sung feelings + actions: stuck → try → do the move → yay.
- GOOD: "Shoes on my feet! / Hop hop hop! / Out we go!"
- BAD: "Mom kneels by me. Wave open arms. I stretch and yawn. We dance side by side."
NEVER write hugs, kisses, embraces, holding, wrapping arms, hand-holding, high-fives, cheek-to-cheek, or "pull close".

MINI STORY ARC (still a SONG):
1. PROBLEM (Intro) — short "oh no / can't / stuck" chant
2. DISCOVERY (Verse 1) — simple idea / try
3. FUN (Chorus) — movement hook kids can shout
4. MORE FUN (Verse 2) — same hook, one new word/place
5. CELEBRATION (Outro) — done / yay / wave (warm resolve) — NO hug

HARD RULES:
- Setting must be depictable in: kitchen, kitchen_sink, lawn, bedroom, home, dining_room, playroom, backyard, bathtub, porch, living_rug
- GEOGRAPHY LOCK: lyric places MUST match the objective/theme/allowed rooms.
  - Wash/kitchen/eat: NEVER bed, blanket, sheets, pillow, lawn, yard, outdoor play. Problem = dirty hands / can't eat yet / need soap.
  - Stomp/clap/outdoor/yard/backyard/lawn: NEVER bed, blanket, sheets, pillow, sink, soap. Problem = can't stomp yet / shoes stuck / need to move outside.
  - Bedtime only: soft bed words OK; no stomping.
- At most ONE place name per section (don't tour every room in lyrics)
- Complete every lyric line (never cut mid-sentence)
- Bedtime: soft words; no stomping
...
- Cast: prefer "I"/"my"; only name cast members that are listed in SETUP LOCKS — never invent Mom/Sasha/Adam if they are not in the cast
- If cast is a single toddler, the song is SOLO — no Mom, no second child
...
- Lead toddler (named in SETUP / cast — examples historically used "Adam") REQUIRED on every beat — drives the journey
- Helper Mom OPTIONAL only when cast includes a helper (~1/3–1/2 of beats when lyrics/theme say Mom/help). Never invent a helper who is not in the cast.
- Prefer INTERACTION over solo when a helper is present; otherwise solo lead actions only
...
- BOTH lead+helper: lead left/center, helper right — never same slot.
- SCREEN CONTINUITY (toddler is continuity lead):
  - Same room: keep lead placement; NEVER left↔right teleport for variety
  ...
  "beats": [
    {
      ...
      "storyBeat": "problem",
      "cause": "song starts",
      "effect": "can't start the move yet",
      "interaction": "Lead looks around then gets ready",
      "lyricHint": "can't move yet",
      "cutMotivation": "look",
      "actionPhase": "setup",
      "beatRole": "setup",
...
const ENERGETIC_MOTION = {
  ...
  wash: "rubbing palms under the faucet washing hands on the beat, FIVE clear fingers each hand, soap splash, keep hands readable not melted not fused, preschool wash gesture, standing on FLOOR not on counter",
...
const CALM_MOTION = {
  ...
  wash: "gentle hand washing under water, soft palm rubs, keep five fingers clear on each hand, standing on floor",
...
export function kidsHitLocationPalette(theme, allowed = []) {
  ...
  if (/lawn|outside|shoes|hop|yard|backyard|stomp|clap/.test(t))
    return keep("backyard", "lawn", "porch", "home", "doorway", "hallway");
...
/**
 * Drop garbage / mid-cut trailing lyric lines (e.g. lone "H").
 */
export function dropJunkLyricLines(lyrics) {
  ...
  for (const line of lines) {
    ...
    if (/^\[/.test(t) || /^TITLE:/i.test(t) || /^OBJECTIVE:/i.test(t) || /^LYRICS:/i.test(t)) {
      out.push(t);
      continue;
    }
    // Lone letter / fragment cutoffs
    if (t.length <= 2 && !/^(i|we|oh|no|go|up|on)$/i.test(t)) continue;
    if (/^(h|th|a|an|the|and|to|for|my|i'?m|we'?re)\s*$/i.test(t)) continue;
...
/**
 * If the model omitted [Intro]/[Chorus] headers, wrap plain lines into the
 * required kids-hit structure so parsing / ACE / beat planning can continue.
 */
export function ensureKidsLyricStructure(lyrics, { objective = "", theme = "" } = {}) {
  let text = dropJunkLyricLines(normalizeKidsLyricsText(lyrics));
  if (!text.trim()) return text;

  if (/\[(Intro|Verse|Chorus|Outro)/i.test(text)) {
    // Ensure outro has at least one line
    const outroBody = /\[Outro\]\s*([\s\S]*?)(?=\n\[|$)/i.exec(text)?.[1] || "";
    if (!outroBody.trim()) {
      const celebrate = celebrationLineFor(theme, objective);
      text = text.replace(/\[Outro\]\s*(?=\n\[|$)/i, `[Outro]\n${celebrate}\n`);
      if (!/\[Outro\]/i.test(text)) {
        text = `${text.trimEnd()}\n\n[Outro]\n${celebrate}`;
      }
    }
    return text.trim();
  ...
  const body = text
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l && !/^TITLE:/i.test(l) && !/^OBJECTIVE:/i.test(l) && !/^LYRICS:/i.test(l));

  if (!body.length) return text;

  const n = body.length;
  // Split into 6 buckets: Intro, V1, Chorus, V2, Chorus, Outro
  const cuts = [
    0,
    Math.max(1, Math.round(n * 0.15)),
    Math.max(2, Math.round(n * 0.3)),
    Math.max(3, Math.round(n * 0.5)),
    Math.max(4, Math.round(n * 0.65)),
    Math.max(5, Math.round(n * 0.85)),
    n,
  ];
  // Ensure strictly increasing cut points
  for (let i = 1; i < cuts.length; i++) {
    if (cuts[i] <= cuts[i - 1]) cuts[i] = Math.min(n, cuts[i - 1] + 1);
  }
  cuts[cuts.length - 1] = n;

  const slice = (a, b) => body.slice(a, b);
  const parts = [
    ["Intro", slice(cuts[0], cuts[1])],
    ["Verse 1", slice(cuts[1], cuts[2])],
    ["Chorus", slice(cuts[2], cuts[3])],
    ["Verse 2", slice(cuts[3], cuts[4])],
    ["Chorus", slice(cuts[4], cuts[5])],
    ["Outro", slice(cuts[5], cuts[6])],
  ];

  // Guarantee non-empty sections by borrowing from neighbors
  for (let i = 0; i < parts.length; i++) {
    if (parts[i][1].length) continue;
    const donor = parts.find((p) => p[1].length > 1);
    if (donor) {
      parts[i][1].push(donor[1].pop());
    } else if (body.length) {
      parts[i][1].push(body[Math.min(i, body.length - 1)]);
    }
  }
  if (!parts[5][1].length) {
    parts[5][1].push(celebrationLineFor(theme, objective));
  }
...
function celebrationLineFor(theme, objective) {
  const t = `${theme} ${objective}`.toLowerCase();
  if (/shoe|outside|lawn|hop/i.test(t)) return "Shoes on we go!";
  if (/bed|sleep|yawn|night/i.test(t)) return "Cozy and ready!";
  if (/wash|hands|kitchen/i.test(t)) return "Clean hands ready!";
  if (/tidy|toys/i.test(t)) return "Toys all tidy!";
  if (/dance|freeze/i.test(t)) return "Dance then freeze!";
  if (/stomp|clap/i.test(t)) return "Stomp and clap done!";
  if (/rain|march/i.test(t)) return "March inside done!";
  return "We did it today!";
}
...
export function kidsHitMotionPrompt({
  ...
  const parts = poses.map((p) =>
    poseMotionForContinuity(p, { energetic, phase, hasPrev: hasPrev || frameChained }),
  );
  ...
  const chainLock = frameChained
    ? [
        "PIXEL CONTINUITY: continue from THIS exact starting frame",
        "lock the room background, camera angle, body placement, scale, and outfits",
        "evolve only the action and expression — no teleport, no hard cut reset, no pose snap",
        castChanged && helperPresent
          ? "a helper may gently step into frame near the toddler if not already present"
          : castChanged && !helperPresent
            ? "extra people may gently leave frame — do not teleport the toddler"
            : helperPresent
              ? "helper already in frame if shown — keep both identities fixed"
              : "only characters already in the still — no new person suddenly pops in",
      ].join("; ")
    : null;
  const storyBridge =
    !frameChained &&
    hasPrev &&
    (prevEffect || cause || nowEffect)
      ? [
  ...
  return [
    ...
    helperPresent
      ? "CAST LOCK: exactly TWO people only — one toddler boy Adam and one adult mom Sasha — count them: 2 — no twin, no clone, no daughter, no second child, no child in sink, no glasses"
      : "CAST LOCK: exactly ONE toddler boy Adam alone — count them: 1 — no mom, no twin, no clone, no second child, no glasses, no extra person",
    "FEET ON FLOOR: characters stand on floorboards only — NEVER sit or stand on counters, tables, sinks, or furniture tops",
    helperPresent
    ...
    "NO kiss, NO hug, NO embrace, NO extra people, NO second child, NO twin, NO clone, NO glasses, NO sitting on counter",
...
function poseForStory(hint, { mood, section, theme, storyBeat } = {}) {
  ...
  const washTheme = /wash|soap|hands|kitchen|sink|scrub|rinse/.test(
    String(theme || ""),
  );
  // Arc soft overrides when lyric doesn't force a strong action
  const forced =
    /\b(clap|stomp|wave|tiptoe|march|wash|splash|soap|rinse|scrub|point|stretch|yawn|walk|go|hug|run|kneel|look)\b/i.test(
      String(hint || ""),
    );
  if (!forced) {
    if (arc === "problem") pose = mood === "calm" ? "sit" : "stand";
    else if (arc === "discovery") pose = washTheme ? "wash" : "point";
    else if (arc === "fun") {
      if (washTheme) pose = "wash";
      else if (/stomp|march/.test(String(theme || "")))
        pose = /stomp/.test(theme) ? "stomp" : "walk";
      else pose = "clap";
    } else if (arc === "celebration") pose = washTheme ? "wave" : "hands_up";
  }
  if (arc === "problem" && (pose === "stomp" || pose === "hands_up")) pose = "stand";
  if (arc === "celebration" && pose === "stand") pose = "wave";
  if (washTheme && (pose === "clap" || pose === "stomp") && arc === "fun") pose = "wash";
  return pose;
}
...
function momPoseForStory(hint, arc, mood) {
  ...
  if (/wash|soap|scrub|rinse|sink|hands/.test(h)) return "point";
  if (/kneel|welcome|open arms/.test(h) || a === "celebration") return "kneel";
  ...
  if (a === "discovery") return "point";
  if (a === "fun") return "wave";
...
/** Cast names present on a beat (characters[] + placement keys). */
export function beatCastNames(beat) {
  const names = new Set();
  if (!beat || typeof beat !== "object") return names;
  ...
  for (const k of Object.keys(beat.placement || {})) {
    const n = String(k || "")
      .toLowerCase()
      .trim();
    if (n) names.add(n);
...
/**
 * Hybrid last-frame chain: seed Wan clip N+1 from clip N's end frame
 * when the shot continues in the same room family (kitchen ↔ kitchen_sink).
 *
 * Hard resets (authored keyframe) for bridge changes and exit cuts.
 * Same-room cast changes (Mom enter/exit) still chain — blend with the next
 * keyframe so the new person can appear without a hard cut.
 */
export function shouldChainFromPrevClip(prevBeat, beat) {
  if (!prevBeat || !beat) return false;
  if (beat.bridge === true || prevBeat.bridge === true) return false;

  const prevLoc = String(prevBeat.location || "")
    .toLowerCase()
    .trim();
  const loc = String(beat.location || "")
    .toLowerCase()
    .trim();
  if (!prevLoc || !loc) return false;
  if (isBridgeLoc(loc) || isBridgeLoc(prevLoc)) return false;
  // Same exact room OR same family (kitchen → kitchen_sink approach)
  if (prevLoc !== loc && roomFamily(prevLoc) !== roomFamily(loc)) return false;

  const cut = String(beat.cutMotivation || "")
    .toLowerCase()
    .trim();
  // Leaving the room → next clip should plant from its authored keyframe
  if (cut === "exit" || prevBeat.cutMotivation === "exit") return false;

  return true;
}

/** True when Mom (or any helper) appears/disappears between neighboring beats. */
export function castChangedBetweenBeats(prevBeat, beat) {
  return !castSetsEqual(beatCastNames(prevBeat), beatCastNames(beat));
}

/**
 * Keep helper (Mom) on for 3–4 consecutive same-room beats once she appears,
 * and stay out for at least 2 beats after she leaves — reduces cast flicker.
 */
export function stabilizeCastBlocks(
  beats,
  { minOn = 3, maxOn = 4, minOff = 2, leadName = "Adam", helperName = null } = {},
) {
  const list = Array.isArray(beats) ? beats.map((b) => ({ ...b })) : [];
  if (list.length < 2) return list;
  // Solo cast — never inject a helper
  if (!helperName) {
    for (const beat of list) {
      const lead =
        beat.characters?.find((c) => c?.name === leadName) ||
        beat.characters?.[0] || {
          name: leadName,
          pose: "stand",
          expression: "curious",
      ...
      beat.characters = [{ ...lead, name: leadName }];
      if (beat.placement) {
        const slot =
          beat.placement[leadName] ||
          beat.placement.Adam ||
          beat.placement.adam ||
          Object.values(beat.placement)[0] ||
          "mid_left";
        beat.placement = { [leadName]: slot };
      }
    }
    return list;
  }

  const helperRe = new RegExp(`^${helperName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");
  const leadRe = new RegExp(`^${leadName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i");

  const hasHelper = (b) =>
    !!(
      b?.characters?.some((c) => helperRe.test(c?.name)) ||
      b?.placement?.[helperName] ||
      b?.placement?.Sasha ||
      b?.placement?.sasha
    );

  const cloneHelper = (from, into) => {
    const src =
      from.characters?.find((c) => helperRe.test(c?.name)) || {
        name: helperName,
        pose: "stand",
    ...
    const lead =
      into.characters?.find((c) => leadRe.test(c?.name)) ||
      into.characters?.[0] || {
        name: leadName,
        pose: "stand",
        expression: "curious",
    ...
    into.placement = {
      ...(into.placement || {}),
      [leadName]:
        into.placement?.[leadName] ||
        into.placement?.Adam ||
        "mid_left",
      [helperName]:
        into.placement?.[helperName] ||
        into.placement?.Sasha ||
        "mid_right",
    };
    into.proximity = into.proximity || "near";
  };

  const dropHelper = (into) => {
    const lead =
      into.characters?.find((c) => leadRe.test(c?.name)) ||
      into.characters?.[0] || {
        name: leadName,
    ...
    into.characters = [{ ...lead, name: leadName }];
    if (into.placement) {
      const next = { ...into.placement };
      delete next[helperName];
      delete next.Sasha;
      delete next.sasha;
      if (!next[leadName] && (next.Adam || next.adam)) {
        next[leadName] = next.Adam || next.adam;
        delete next.Adam;
        delete next.adam;
      }
      into.placement = next;
    }
    if (into.proximity === "near" || into.proximity === "close") {
      into.proximity = "apart";
    }
  };

  let i = 0;
  while (i < list.length) {
    if (list[i].bridge || isBridgeLoc(list[i].location)) {
      i += 1;
      continue;
    }
    if (!hasHelper(list[i])) {
    ...
    const loc = String(list[i].location || "").toLowerCase();
    let end = i;
    while (
      end + 1 < list.length &&
      !list[end + 1].bridge &&
      !isBridgeLoc(list[end + 1].location) &&
      String(list[end + 1].location || "").toLowerCase() === loc &&
      end + 1 - i < maxOn
    ) {
      end += 1;
    }
    // Extend ON block to at least minOn within same room
    while (
      end - i + 1 < minOn &&
      end + 1 < list.length &&
      !list[end + 1].bridge &&
    ...
    const donor = list[i];
    for (let j = i; j <= end; j++) cloneHelper(donor, list[j]);

    // Enforce minOff after the block in the same room
    let off = 0;
    let k = end + 1;
    while (
      k < list.length &&
      off < minOff &&
      !list[k].bridge &&
      !isBridgeLoc(list[k].location) &&
      String(list[k].location || "").toLowerCase() === loc
    ) {
      dropHelper(list[k]);
      off += 1;
      k += 1;
...
export function clampBeatCount(beats, durationSec, min = KIDS_HIT_BEAT_MIN, max = KIDS_HIT_BEAT_MAX) {
  ...
  while (list.length > max) {
    ...
    for (let i = 0; i < list.length - 1; i++) {
      ...
      let score = 0;
      if (a.location === b.location) score += 4;
      if (poseA === poseB) score += 2;
      if (String(a.section) === String(b.section)) score += 1;
      // Prefer merging non-exit beats; same-room always attractive
      if (a.cutMotivation === "exit" || b.cutMotivation === "exit") score -= 2;
      // Prefer merging when cast matches (avoid mid-block Mom flicker merges wrongly)
      const castA = beatCastNames(a);
      const castB = beatCastNames(b);
      if (castSetsEqual(castA, castB)) score += 2;
      if (score > bestScore) {
        bestScore = score;
        best = i;
      }
...
export function poseFromLyricHint(hint, { mood = "energetic", section = "", theme = "" } = {}) {
  ...
  const washTheme = /wash|soap|hands|kitchen|sink|scrub|rinse/.test(th);
  ...
  // Wash / soap / rinse MUST NOT collapse to clap
  if (/wash|soap|scrub|splash|rinse|bubble|suds|clean hands|dirty hands|\bhands\b/.test(h))
    return "wash";
  ...
  if (/\bstomp\b/.test(h) && mood !== "calm" && !washTheme) return "stomp";
  if (/\bclap\b/.test(h)) return "clap";
  if (/chorus/.test(sec) && mood !== "calm") {
    if (washTheme) return "wash";
...
export function repairKidsHitBeats(
  ...
  const lead = String(leadName || "Adam");
  const helper = helperName ? String(helperName) : null;
  ...
  const washTheme = /wash|soap|hands|kitchen|sink|scrub|rinse/.test(
    String(theme || "") + " " + String(objective || ""),
  );
  const poseAlts = {
    problem: mood === "calm" ? ["sit", "stand", "tiptoe"] : ["stand", "point", "wave"],
    discovery: washTheme
      ? ["wash", "point", "wave"]
      : ["point", "walk", "clap", "wave"],
    fun:
      mood === "calm"
        ? ["tiptoe", "wave", "hands_up"]
        : washTheme
          ? ["wash", "wave", "point", "hands_up"]
          : ["clap", "walk", "stomp", "hands_up", "wave"],
    celebration: washTheme
      ? ["wave", "hands_up", "wash", "stand"]
      : ["hands_up", "wave", "clap", "stand"],
  };
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    // Only honor lyric→room when that room is in the selected palette
    if (
      hintLoc &&
      paletteSet.has(hintLoc) &&
      !(indoor && hintLoc === "lawn")
    ) {
      loc = hintLoc;
    }
    ...
    if (
      /wash|soap|splash|rinse|scrub|suds|sink/.test(hint.toLowerCase()) &&
      arc !== "celebration" &&
      arc !== "problem"
    ) {
      // Prefer sink for wash action, but do not monopolize every discovery/fun beat
      if (i % 2 === 0 && paletteSet.has("kitchen_sink")) loc = "kitchen_sink";
      else if (paletteSet.has("kitchen") && arc === "discovery" && i % 3 === 1)
        loc = "kitchen";
      else if (paletteSet.has("kitchen_sink")) loc = "kitchen_sink";
      else if (paletteSet.has("kitchen")) loc = "kitchen";
    }
    // Celebration prefers last story room (often home) over sink spam
    if (arc === "celebration" && storyRooms.length) {
      loc = storyRooms[storyRooms.length - 1];
    }
    // Last ~20% of song: force home/celebration room when available
    if (
      storyRooms.length >= 2 &&
      i >= Math.max(1, Math.floor(list.length * 0.8)) &&
      paletteSet.has(storyRooms[storyRooms.length - 1])
    ) {
      loc = storyRooms[storyRooms.length - 1];
      if (arc !== "celebration") beat.storyBeat = "celebration";
    }
    ...
    // Keep active lead; optionally keep/inject helper only when cast includes one
    const helperRe = helper
      ? new RegExp(`^${helper.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i")
      : null;
    ...
    let toddler =
      chars.find((c) => leadRe.test(c?.name)) ||
      chars.find((c) => /^adam$/i.test(c?.name)) ||
      chars.find((c) => !(helperRe && helperRe.test(c?.name)) && !/^sasha$/i.test(c?.name)) ||
      null;
    let helperChar =
      (helper && chars.find((c) => helperRe.test(c?.name))) ||
    ...
    toddler.name = lead;
    toddler.pose = pose;
    toddler.expression = expressionForStory(arc, hint, mood);
    toddler.facing = "front";
    ...
    beat.characters = helperChar ? [toddler, helperChar] : [toddler];

    // Initial placement: lead continuity; helper close when present
    let slot = i === 0 ? "mid_left" : "mid_left";
    let depth = "mid";
    if (arc === "celebration") depth = "near";
    if (loc === "kitchen_sink") depth = "far";
    beat.placement = { [lead]: slot };
    if (helperChar) {
      beat.placement[helper] = "mid_right";
      beat.proximity = "near";
    }
    beat.depth = depth;
  ...
  // Hold helper on for multi-beat blocks (same room) instead of flicker in/out
  list = stabilizeCastBlocks(list, {
    minOn: 3,
    maxOn: 4,
    minOff: 2,
    leadName: lead,
  ...
  list = scrubStaleGeographyText(list, { indoor, objective, theme });
  list = rebuildCauseEffectChain(list, { objective });
  list = enforceLivingStory(list, { mood, objective, theme });
  list = applyCameraShotCards(list);
  list = stampEndPlacements(list, lead);
  ...
  if (musicMap?.beats?.length) {
    list = snapBeatTimingsToMusic(
      list,
      musicMap,
      durationSec || KIDS_HIT_DURATION_SEC,
    );
  ...
  // Hard clamp: never leave a room the user didn't select
  if (allowedLocations?.length) {
    const allow = new Set(allowedLocations.map((id) => String(id).toLowerCase()));
    const fallback = String(allowedLocations[0]).toLowerCase();
    list = list.map((b) => {
      const loc = String(b.location || "").toLowerCase();
      if (allow.has(loc)) return b;
      return { ...b, location: fallback, bridge: false };
    });
  }
...
/** Rewrite cause/effect/hints that still mention rooms we left or outdoor when indoor. */
function scrubStaleGeographyText(
  beats,
  { indoor = false, objective = "", theme = "" } = {},
) {
  const washObj = /wash|hands|soap|kitchen|sink|eat/i.test(String(objective || ""));
  const outdoorObj = /stomp|clap|yard|backyard|lawn|outdoor|shoes|hop|march/i.test(
    `${objective} ${theme}`,
  );
  return (beats || []).map((b) => {
    const beat = { ...b };
    const loc = String(beat.location || "").replace(/_/g, " ");
    const kitchenFamily = /kitchen|sink/.test(String(beat.location || "").toLowerCase());
    const outdoorLoc = /backyard|lawn|porch/.test(
      String(beat.location || "").toLowerCase(),
    );
    const scrub = (s) => {
      let t = String(s || "");
      if (indoor) {
      ...
      if (washObj || kitchenFamily) {
        t = t
          .replace(/\bstuck in bed( now)?\b/gi, "dirty hands now")
          .replace(/\bblanket too tight\b/gi, "can't eat yet")
          .replace(/\bin bed\b/gi, "at the sink")
          .replace(/\bblanket\b/gi, "soap")
          .replace(/\bsheets?\b/gi, "hands")
          .replace(/\bpillow\b/gi, "towel");
      }
      if (outdoorObj || outdoorLoc) {
        t = t
          .replace(/\bstuck in bed( now)?\b/gi, "can't stomp yet")
          .replace(/\bblanket too tight\b/gi, "shoes feel stuck")
          .replace(/\bin bed\b/gi, `in the ${loc || "yard"}`)
          .replace(/\bblanket\b/gi, "shoes")
          .replace(/\bsheets?\b/gi, "feet")
          .replace(/\bpillow\b/gi, "ball")
          .replace(/\bat the sink\b/gi, `in the ${loc || "yard"}`)
          .replace(/\bdirty hands\b/gi, "heavy feet");
      }
      t = t
    ...
    beat.lyricHint = scrub(beat.lyricHint);
    beat.interaction = scrub(beat.interaction);
    return beat;
  });
}
 ...
 */
function stampEndPlacements(beats, leadName = "Adam") {
  const list = Array.isArray(beats) ? beats : [];
  const lead = String(leadName || "Adam");
  const slotOf = (obj) =>
    obj?.[lead] || obj?.Adam || obj?.adam || Object.values(obj || {})[0] || "center";
  for (let i = 0; i < list.length; i++) {
    const cur = list[i];
    const next = list[i + 1];
    const startSlot = slotOf(cur.placement);
    const traveling =
      cur.cutMotivation === "exit" || cur.bridge === true;
    if (!next) {
      cur.endPlacement = { [lead]: startSlot };
      continue;
    }
    if (traveling) {
      let exit =
      ...
      if (cur.bridge && exit === "center") {
        ...
        cur.enterDir = exit;
        if (startSlot === "center") {
          cur.placement = { [lead]: exit };
        }
      }
      cur.endPlacement = { [lead]: exit };
    } else {
      cur.endPlacement = { [lead]: startSlot };
    }
  }
  ...
  for (let i = 1; i < list.length; i++) {
    const prev = list[i - 1];
    const beat = list[i];
    const aEnd = slotOf(prev.endPlacement) || slotOf(prev.placement) || "center";
    const same =
      roomFamily(prev.location) === roomFamily(beat.location);
    if (beat.bridge || same) {
      if (!beat.placement) beat.placement = {};
      const nextSlot = aEnd === "center" && beat.bridge ? "right" : aEnd;
      beat.placement[lead] = nextSlot;
      // Drop legacy Adam key if remapped
      if (lead !== "Adam") {
        delete beat.placement.Adam;
        delete beat.placement.adam;
      }
      if (beat.bridge) {
        beat.enterDir = nextSlot;
        beat.exitDir = nextSlot;
        beat.endPlacement = { [lead]: nextSlot };
...
export function finalizeKidsHitPlan(
  plan,
  { theme, allowedLocations, durationSec, lyricsText, musicMap = null } = {},
) {
  ...
  const beats = repairKidsHitBeats(plan.beats || [], {
    ...
    musicMap: musicMap || plan.musicMap || null,
  });
  const out = {
    ...plan,
    kidsHit: true,
