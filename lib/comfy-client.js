...
/** Cache Comfy /object_info lookups per URL + node class. */
const objectInfoCache = new Map();

/**
 * Fetch Comfy node schema. Pass classType for a single node, or omit for all.
 */
export async function comfyObjectInfo(url, classType = null) {
  const base = String(url || "").replace(/\/$/, "");
  const key = classType ? `${base}::${classType}` : `${base}::*`;
  if (objectInfoCache.has(key)) return objectInfoCache.get(key);
  const path = classType
    ? `/object_info/${encodeURIComponent(classType)}`
    : "/object_info";
  const info = await comfy(base, path, { timeoutMs: 30000, retries: 2 });
  objectInfoCache.set(key, info);
  return info;
}

/** True when the remote Comfy node declares an input (required or optional). */
export async function comfyNodeHasInput(url, classType, inputName) {
  try {
    const info = await comfyObjectInfo(url, classType);
    const node = info?.[classType] || info;
    const required = node?.input?.required || {};
    const optional = node?.input?.optional || {};
    return (
      Object.prototype.hasOwnProperty.call(required, inputName) ||
      Object.prototype.hasOwnProperty.call(optional, inputName)
    );
  } catch (err) {
    console.warn(
      `  Comfy object_info ${classType}.${inputName}: ${err.message || err}`,
    );
    return false;
  }
}
