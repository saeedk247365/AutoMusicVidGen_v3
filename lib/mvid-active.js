...
export async function saveActiveProject(state) {
  ...
  const doc = {
    ...
    setup: state.setup && typeof state.setup === "object" ? state.setup : undefined,
...
/** Draft cast/rooms selection before a song folder exists (survives refresh). */
export const SETUP_DRAFT_PATH = join(ROOT, "batches", ".mvid-setup-draft.json");

export async function loadSetupDraft() {
  try {
    if (!existsSync(SETUP_DRAFT_PATH)) return null;
    const data = JSON.parse(stripBom(await readFile(SETUP_DRAFT_PATH, "utf8")));
...
export async function saveSetupDraft(setup) {
  if (!setup || typeof setup !== "object") return null;
  await mkdir(dirname(SETUP_DRAFT_PATH), { recursive: true });
  const doc = {
    castIds: Array.isArray(setup.castIds) ? setup.castIds : [],
    locationIds: Array.isArray(setup.locationIds) ? setup.locationIds : [],
    locationsExplicit: setup.locationsExplicit === true,
    roomsLockedByUser: setup.roomsLockedByUser === true,
    title: String(setup.title || ""),
    objective: String(setup.objective || ""),
    theme: String(setup.theme || ""),
    noFrameChain: setup.noFrameChain === true,
    frameChain: setup.frameChain !== false && setup.noFrameChain !== true,
    updatedAt: new Date().toISOString(),
  };
  await writeFile(SETUP_DRAFT_PATH, JSON.stringify(doc, null, 2), "utf8");
  return doc;
}

export async function clearSetupDraft() {
  try {
    if (existsSync(SETUP_DRAFT_PATH)) await unlink(SETUP_DRAFT_PATH);
  } catch {
    /* ignore */
  }
}
