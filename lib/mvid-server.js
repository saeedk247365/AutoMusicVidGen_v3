...
import {
  ...
  importKeyframeBank,
  deleteCharacter,
  keyframePoseGuideStatus,
  prepareTrainingImagesFromKeyframes,
  alignStyleFamilyCheckpoint,
  CHARACTER_PRESETS,
  getJob,
...
export function createMvidServer(orchestrator, { port = 3847 } = {}) {
  ...
  app.put("/api/characters/:id", async (req, res) => {
    try {
      ...
      let character = await saveCharacter(id, patch);
      const styleAlign = await alignStyleFamilyCheckpoint(id);
      if (styleAlign?.changed) {
        character = (await loadCharacter(id)) || character;
      }
      res.json({
        ok: true,
        character,
  ...
  app.get("/api/character-pose-guide", async (req, res) => {
    try {
      const id = req.query?.character || req.query?.id || "";
      const result = await keyframePoseGuideStatus(id);
      res.json(result);
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message || String(err) });
    }
  ...
  app.post("/api/characters/:id/keyframes/import-bank", async (req, res) => {
    try {
      const body = req.body || {};
      const files = Array.isArray(body.files) ? body.files : [];
      const result = await importKeyframeBank(req.params.id, files);
      res.status(result.ok ? 200 : 400).json(result);
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message || String(err) });
    }
  ...
  app.post("/api/characters/:id/keyframes/:keyframeId/upload", async (req, res) => {
    try {
      const body = req.body || {};
      const b64 = String(body.imageBase64 || "").replace(/^data:[^;]+;base64,/, "");
      if (!b64) {
      ...
      const buffer = Buffer.from(b64, "base64");
      const result = await importKeyframe(
        req.params.id,
        req.params.keyframeId,
        buffer,
      );
  ...
  app.post("/api/characters/:id/prepare-training", async (req, res) => {
    try {
      const result = await prepareTrainingImagesFromKeyframes(req.params.id);
      res.status(result.ok ? 200 : 400).json(result);
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message || String(err) });
    }
  ...
  app.use((err, _req, res, next) => {
    ...
    if (tooLarge) {
      return res.status(413).json({
        ok: false,
        error:
          "Upload too large. Import fewer images at once, or use smaller PNGs.",
      });
    }
