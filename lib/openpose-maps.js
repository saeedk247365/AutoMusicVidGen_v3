...
export const ANGLE_YAW = {
  front: 0,
  threequarter_soft_left: -30,
  threequarter_left: -55,
  threequarter_soft_right: 30,
...
/**
 * ControlNet pose maps use a stronger yaw than the semantic ANGLE_YAW so
 * OpenPose can beat front latents at low denoise (≤0.60). Ladder math still
 * uses ANGLE_YAW.
 */
export const CONTROLNET_YAW = {
  front: 0,
  threequarter_soft_left: -55,
  threequarter_left: -75,
  threequarter_soft_right: 55,
  threequarter_right: 75,
  side_left: -90,
  side_right: 90,
  threequarter_back_left: -145,
  threequarter_back_right: 145,
...
function drawSkeleton(rgb, joints2d, yawDeg = 0) {
  ...
  if (nose && neck) {
    ...
    // Soft/hard three-quarter: prefer near-side face so ControlNet does not lock a front face.
    const threeQuarterLeft = yawNorm >= 25 && yawNorm <= 70 && yawDeg < 0;
    const threeQuarterRight = yawNorm >= 25 && yawNorm <= 70 && yawDeg > 0;
    ...
    if (rearish && !joints2d._lookOver) {
    ...
    } else if (profileLeft || threeQuarterLeft) {
      ...
      if (threeQuarterLeft) {
        // faint far-side eye only — still biased to the turn
        disc(rgb, rightEye[0], rightEye[1], 2, C.face);
      }
    } else if (profileRight || threeQuarterRight) {
      ...
      disc(rgb, nose[0], nose[1], 9, C.face);
      if (threeQuarterRight) {
        disc(rgb, leftEye[0], leftEye[1], 2, C.face);
      }
    } else {
      disc(rgb, leftEye[0], leftEye[1], 4, C.face);
      disc(rgb, rightEye[0], rightEye[1], 4, C.face);
