...
export const LOCATION_LAYOUT = {
  ...
  kitchen_sink: {
    ...
    slotBias: "center",
    shadow: "indoor",
    note: "near sink but feet planted on FLOOR play-lane, not on counter",
  },
  dining_room: {
    scale: 0.4,
    bottomPad: 0.12,
...
export function resolveCharacterLayout({
  ...
  // Mom slightly taller than toddler (1.18×) — avoid giant floating Mom
  if (isMom) scale = scale * 1.18;

  // Mild camera framing — never jump to near-fullscreen
  if (cam === "close" || cam === "portrait" || camMot === "push_in" || camMot === "lower") {
    scale *= 1.12;
    bottomPad = Math.max(0.06, bottomPad - 0.03);
  } else if (cam === "medium") {
    scale *= 1.06;
  } else if (cam === "medium_full") {
    scale *= 1.03;
  } else if (cam === "full_body" || cam === "wide") {
    scale *= 0.96;
    bottomPad = Math.min(0.16, bottomPad + 0.02);
  }
...
export async function removePlateBackground(inputPathOrBuf) {
  ...
  try {
    ...
    return trimToOpaqueBounds(cleaned, 12);
  } finally {
    try {
...
async function softContactShadow(width, height, mode = "indoor") {
  ...
  const opacity = mode === "outdoor" ? 0.45 : 0.42;
  const rx = mode === "outdoor" ? 0.62 : 0.55;
  const ry = mode === "outdoor" ? 0.4 : 0.48;
  ...
  <defs>
    <radialGradient id="g" cx="${cx}" cy="50%" r="50%">
      ...
      <stop offset="50%" stop-color="black" stop-opacity="${opacity * 0.45}"/>
...
async function featherCutoutEdges(pngBuf) {
  ...
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      ...
      // Stronger fringe kill for sticker halos / gray matte boxes
      if (a < 180) out[o + 3] = Math.max(0, a - 70);
    }
  }
...
/**
 * Crop a region from an image buffer/path and resize to outW×outH.
 */
export async function extractCameraCrop(
  inputPathOrBuf,
  rect,
  { width, height } = {},
) {
  const outW = width || rect.width;
  const outH = height || rect.height;
